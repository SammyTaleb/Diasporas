...
def snippetsParser(id,query,search_engine_nbr=0):
    ...
    try:
        for i in range(len(results)):
            snippets_frame.append({'number_snippet':i,'site':results[i]['links'],'engine_search':search_engine_nbr,
                                   'id_person':id,'search':query,'title':results[i]['titles'],'text':results[i]['descriptions']})
    except:
        pass
